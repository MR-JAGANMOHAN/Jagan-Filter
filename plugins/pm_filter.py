...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url=f'https://telegram.me/{HOW_TO_DOWNLOAD}'),
            InlineKeyboardButton('⚚ 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙈𝙤𝙫𝙞𝙚𝙨 𝙃𝙚𝙧𝙚 ⚚', url=f'https://telegram.me/{REQUEST_MOVIES}')
        ]
    )
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply(f"I couldn't find any movie in that name. \n[⚚ 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙈𝙤𝙫𝙞𝙚𝙨 𝙃𝙚𝙧𝙚 ⚚](https://telegram.me/{REQUEST_MOVIES})")
    ...
    if not movielist:
        k = await msg.reply(f"I couldn't find anything related to that. Check your spelling.\n[⚚ 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙈𝙤𝙫𝙞𝙚𝙨 𝙃𝙚𝙧𝙚 ⚚](https://telegram.me/{REQUEST_MOVIES})")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply(f"I couldn't find anything related to that\nDid you mean any one of these? \n[⚚ 𝙍𝙚𝙦𝙪𝙚𝙨𝙩 𝙈𝙤𝙫𝙞𝙚𝙨 𝙃𝙚𝙧𝙚 ⚚](https://telegram.me/{REQUEST_MOVIES})",
                    reply_markup=InlineKeyboardMarkup(btn))
